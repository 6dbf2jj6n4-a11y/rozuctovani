"""
Import klicu voda z Excel souboru Klice_Voda.xlsx.
...
Pouziti:
  python manage.py import_klice_voda cesta/k/souboru.xlsx --site FM
"""
import openpyxl
...
class Command(BaseCommand):
    help = "Importuje klice voda (AllocationKey) z Excel souboru"
    ...
    def handle(self, *args, **options):
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            meter_code = str(data.get("VODA_KodOM") or "").strip()
            ...
            if not service_item:
                ...
                if ServicePoolItem.objects.filter(site=site, invoice_class="water").count() == 1:
                    service_item = ServicePoolItem.objects.filter(
                        site=site, invoice_class="water", name__icontains="hlavní odběr"
                    ).first()
            ...
            if typ == "K_CELKU":
                ...
                if mplochy and kczam and float(kczam) > 0:
                    ...
                    # mistnost - u souctove castky pres vice ploch (napr. srazky,
                    # kde Mplochy = soucet vymer vsech ploch karty) byva 0/prazdne
